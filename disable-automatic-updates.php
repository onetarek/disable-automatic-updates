...
/*
 * New hooks name update : https://make.wordpress.org/core/author/dd32/
 * https://codex.wordpress.org/Configuring_Automatic_Background_Updates
 * auto_update_{$type} filter, where $type would be replaced with "core", "plugin" , "theme", "translation. 
 * eg : auto_update_core, auto_update_plugin, auto_update_theme, auto_update_translation
 */
...
function disau_set_upgrade_filters()
{
	...
	else 
	{
		...
		if( $options['core'] == 1 ) 
		{
			if( $options['core_dev'] == 1 ) 
			{
				add_filter( 'allow_dev_auto_core_updates', '__return_true', 100 );
			}
			else
			{
		...
		if( $options['plugin'] == 1 )
		{
			add_filter( 'auto_update_plugin', '__return_true', 100 );
		}
		else
		{
		...
		if( $options['theme'] == 1 )
		{
			add_filter( 'auto_update_theme', '__return_true', 100 );
		}
		else
		{
		...
		if( $options['translation'] == 1 )
		{
			add_filter( 'auto_update_translation', '__return_true', 100 );
		}
		else
		{
		...
		if( $options['vcscheck'] == 1 )
		{
			add_filter( 'automatic_updates_is_vcs_checkout', '__return_true', 100 );
		}
		else
		{
			add_filter( 'automatic_updates_is_vcs_checkout', '__return_false', 100 );
		}
	
		add_filter( 'auto_core_update_send_email', 'disau_core_update_email_filter', 100, 2 );


		if( $options['debugemail'] == 1 )
		{	
			add_filter( 'automatic_updates_send_debug_email ', '__return_true', 100 );
		}
		elseif( $options['debugemail'] == 1 )
		{	
			add_filter( 'automatic_updates_send_debug_email ', '__return_false', 100 );
		}

	}


}#end of function disau_set_upgrade_filters()
	...
	<div style="width: 800px; padding-left: 10px;" class="wrap">
	  ...
	  <table class="form-table widefat" width="100%" id="all_other_options" style=" <?php echo ($disau_options['automatic_update']!=1)?' display:none':'' ?>">			
			...
			<tr>
				...
				<br><p style="color:#cccccc;">For Developers: To enable automatic updates even if a VCS folder (.git, .hg, .svn etc) was found in the WordPress directory or any of its parent directories</p>
				</td>
			</tr>
